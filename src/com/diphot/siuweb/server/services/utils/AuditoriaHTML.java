package com.diphot.siuweb.server.services.utils;

import com.diphot.siuweb.server.business.model.Auditoria;

public class AuditoriaHTML {

	public static String getAuditoriaHTML(Auditoria auditoria){
		String html = "<html>\r\n" + 
				"<head></head>\r\n" + 
				"<body style=\"width: 697; height: 1026px; background-color: #acacac; padding: 0px; margin: 0px;\">\r\n" + 
				"	<table style=\"width: 697px; margin-right: auto; margin-left: auto;\">\r\n" + 
				"		<tr style=\"background-color: #c42126; height: 87px;\">\r\n" + 
				"			<td>\r\n" + 
				"				<img style=\"float: right; margin-right: 18px; margin-top: 7px;\" width=\"130px\" height=\"74px\" alt=\"\" src=\"data:image/jpeg;base64,\" />\r\n" +
				"				<strong style=\"float: left; margin-bottom: 20px; font-size: 35px;\">MONITOREO URBANO TIGRE</strong>\r\n" +
				"			</td><!-- fin img_logo -->  \r\n" + 
				"		</tr><!-- fin header -->\r\n" + 
				"		<tr style=\"padding: 0px; margin: 0px;\">\r\n" + 
				"			<td>\r\n" + 
				"				<table style=\"width: 697px; padding: 0px; margin: 0px; background-color: #FFF; float: left;\">\r\n" + 
				"					<tr>  \r\n" + 
				"						<td>\r\n" + 
				"							<p style=\"height: 37px; width: 660px; padding-top: 9px; margin-left: 18px; border-top-width: 1px; border-bottom-width: 1px; border-top-style: solid; border-bottom-style: solid; border-top-color: #bababc; border-bottom-color: #bababc; float: left;\">\r\n" + 
				"							<strong style=\"margin-left: 18px;\"> Informe de Auditotiria n°:  " + auditoria.getId() + " de Inspecci&oacute;n n°:"+ auditoria.getInspeccion().getId()+"</strong></p>\r\n" +
				"							Imagenes Auditoria: "+				
				"						</td><!-- fin titulo -->\r\n" + 
				"					</tr>\r\n" + 
				"					<tr>\r\n" + 
				"						<td>\r\n" + 
				"							<table style=\"margin-left: 18px;\">\r\n" + 
				"								<tr>\r\n"+				
				"								<td style=\"float: left; width: 218px; margin-top: 15px; margin-left: 18px;\">\r\n" + 
				getEncondedImage(1,auditoria)+ 
				"								</td>\r\n" + 
				"								<td style=\"float: left; width: 218px; margin-top: 15px; margin-left: 18px;\">\r\n" + 
				getEncondedImage(1,auditoria)+ 
				"								</td>\r\n" + 
				"								<td style=\"float: left; width: 218px; margin-top: 15px; margin-left: 18px;\">\r\n" + 
				getEncondedImage(3,auditoria)+ 
				"								</td>\r\n" +
				"								</tr>\r\n"+
				"								<tr> <td> Imagenes Inspecci&oacute;n:  </td> </tr>"+
				"								<tr>\r\n"+		
				"								<td style=\"float: left; width: 218px; margin-top: 15px; margin-left: 18px;\">\r\n" + 
				InspeccionHMTL.getEncondedImage(1, auditoria.getInspeccion())+ 
				"								</td>\r\n" + 
				"								<td style=\"float: left; width: 218px; margin-top: 15px; margin-left: 18px;\">\r\n" + 
				InspeccionHMTL.getEncondedImage(1, auditoria.getInspeccion())+ 
				"								</td>\r\n" + 
				"								<td style=\"float: left; width: 218px; margin-top: 15px; margin-left: 18px;\">\r\n" + 
				InspeccionHMTL.getEncondedImage(3, auditoria.getInspeccion())+ 
				"								</td>\r\n" +
				"								</tr>\r\n"+
				"							</table>\r\n" + 
				"						</td>\r\n" + 
				"					</tr>\r\n" + 
				"					<tr>\r\n" + 
				" <!--height: 265px;-->						<td> \r\n" + 
				" <!-- height: 88px;-->							<table style=\"margin-top: 7px; margin-left: 18px; margin-bottom: 15px;\">\r\n" + 
				"								<tr>\r\n" + 
				"									<td style=\"width: 327px; border-right-width: 1px; border-bottom-width: 1px; border-left-width: 1px; 	border-right-style: solid; 	border-bottom-style: solid; 	border-left-style: solid; 	border-right-color: #CCC; 	border-bottom-color: #CCC; 	border-left-color: #CCC; 	-webkit-border-bottom-right-radius: 2px; 	-webkit-border-bottom-left-radius: 2px; 	-moz-border-radius-bottomright: 2px; 	-moz-border-radius-bottomleft: 2px; 	border-bottom-right-radius: 2px; 	border-bottom-left-radius: 2px;\">\r\n" +  
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Detalle Auditoria:  </strong> </p>\r\n" +
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Fecha: </strong>"+ ConversionUtil.getSimpleDate(auditoria.getFecha().toString()) +"</p>\r\n" + 
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Resuelto: </strong>"+ (auditoria.getResuelto() ? "RESUELTO" : " NO RESUELTO") +"</p>\r\n" +
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Observaci&oacute;n: </strong>" + auditoria.getObservaciones()	+"</p>\r\n" + 
				"									</td><!-- fin col_izq -->\r\n" + 
				"									<td style=\"width: 327px; border-right-width: 1px; border-bottom-width: 1px; border-left-width: 1px; 	border-right-style: solid; 	border-bottom-style: solid; 	border-left-style: solid; 	border-right-color: #CCC; 	border-bottom-color: #CCC; 	border-left-color: #CCC; 	-webkit-border-bottom-right-radius: 2px; 	-webkit-border-bottom-left-radius: 2px; 	-moz-border-radius-bottomright: 2px; 	-moz-border-radius-bottomleft: 2px; 	border-bottom-right-radius: 2px; 	border-bottom-left-radius: 2px;\">\r\n" +  
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Detalle Inspeccion:  </strong> </p>\r\n" + 
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Fecha: </strong>"+ ConversionUtil.getSimpleDate(auditoria.getInspeccion().getFecha().toString()) +"</p>\r\n" + 
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Riesgo: </strong>"+ auditoria.getInspeccion().getRiesgo() +"</p>\r\n" +
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Calle: </strong>"+ auditoria.getInspeccion().getCalle() +"</p>\r\n" +
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Altura: </strong>"+ auditoria.getInspeccion().getAltura() +"</p>\r\n" +
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Localidad: </strong>"+ auditoria.getInspeccion().getLocalidad().getNombre() +"</p>\r\n" +
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Entre Calles: </strong>"+ auditoria.getInspeccion().getEntreCalleUno() + " y " + auditoria.getInspeccion().getEntreCalleDos()  +"</p>\r\n" +
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Tema: </strong>"+ auditoria.getInspeccion().getTema().getNombre() +"</p>\r\n" + 
				"										<p style=\"margin-left: 6px; margin-right: 6px; font: 10px sans-serif;\"><strong>Observaci&oacute;n: </strong>" + auditoria.getInspeccion().getObservacion()	+"</p>\r\n" + "									</td><!-- fin col_izq -->\r\n" + 
				"									<td style=\"float: right; width : 325px;\">\r\n" + 
				InspeccionHMTL.getEncondedImage(4,auditoria.getInspeccion())+ 
				"									</td><!-- fin col_der --> \r\n" + 
				"								</tr>\r\n" + 
				"							</table>\r\n" + 
				"						</td>\r\n" + 
				"					</tr><!-- fin contenido -->\r\n" + 
				"				</table>\r\n" + 
				"			</td>\r\n" + 
				"		</tr>\r\n" + 
				//"		<tr style=\"height: 97px; width: 697px; background-color: #ebebeb; float: left;\">\r\n" + 
				//"			<td></td>\r\n" + 
				//"		</tr><!-- fin footer -->\r\n" + 
				"	</table><!-- fin content -->\r\n" + 
				"</body>\r\n" + 
				"</html>\r\n" + 
				"";
		return html;
	}
	
	
	private static String getEncondedImage(Integer number, Auditoria auditoria){
		switch (number) {
		case 1:
			if (auditoria.getEncodedIMG1() != null) {  
				return "<img src=\""+ "data:image/jpeg;base64," + auditoria.getEncodedIMG1().getEncodedImageString() +"\"></img>"; 
			} else { 
				return "";
			}
		case 2:
			if (auditoria.getEncodedIMG2() != null) {  
				return "<img src=\""+ "data:image/jpeg;base64," + auditoria.getEncodedIMG2().getEncodedImageString() +"\"></img>"; 
			} else { 
				return "";
			}
		case 3:
			if (auditoria.getEncodedIMG3() != null) {  
				return "<img src=\""+ "data:image/jpeg;base64," + auditoria.getEncodedIMG3().getEncodedImageString() +"\"></img>"; 
			} else { 
				return "";
			}
		default:
			return "";
		}
	}
	
}
